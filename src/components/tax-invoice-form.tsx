...
export function TaxInvoiceForm({ jobId, editDocId }: { jobId: string | null, editDocId: string | null }) {
  ...
  useEffect(() => {
    if (docToEdit) {
      ...
      form.setValue('items', [{ description: job.technicalReport || job.description, quantity: 1, unitPrice: 0, total: 0 }]);
  ...
  const isCustomerSelectionDisabled = !!jobId || (isEditing && !!docToEdit?.customerId) || isLocked;
