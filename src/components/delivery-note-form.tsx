        ...
        <Form {...form}>
          <form onSubmit={(e) => e.preventDefault()} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Card>
                <CardHeader><CardTitle>1. ข้อมูลลูกค้า</CardTitle></CardHeader>
                <CardContent className="space-y-4">
                  {!jobId && !isEditing && (
                    <FormField control={form.control} name="jobId" render={({ field }) => (
                      <FormItem>
                        <FormLabel>อ้างอิงงานซ่อม</FormLabel>
                        <Popover open={isJobPopoverOpen} onOpenChange={setIsJobPopoverOpen}>
                          <PopoverTrigger asChild>
                            <Button variant="outline" className="w-full justify-between">
                              {field.value ? `งาน: ${field.value.substring(0,8)}` : "เลือกงานที่เสร็จแล้ว..."}
                              <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                            </Button>
                          </PopoverTrigger>
                          <PopoverContent className="w-[--radix-popover-trigger-width] p-0">
                            <div className="p-2 border-b">
                              <Input placeholder="ค้นหา..." value={jobSearch} onChange={e=>setJobSearch(e.target.value)} />
                            </div>
                            <ScrollArea className="h-60">
                              {jobsReadyToBill.filter(j => j.customerSnapshot.name.includes(jobSearch)).map(j=>(
                                <Button key={j.id} variant="ghost" onClick={()=>{field.onChange(j.id); form.setValue('customerId',j.customerId); form.setValue('receiverName',j.customerSnapshot.name); setIsJobPopoverOpen(false);}} className="w-full justify-start h-auto p-2 border-b text-left">
                                  <div className="flex flex-col">
                                    <span className="font-semibold">{j.customerSnapshot.name}</span>
                                    <span className="text-[10px]">{j.description}</span>
                                  </div>
                                </Button>
                              ))}
                            </ScrollArea>
                  ...
                  <FormField control={form.control} name="customerId" render={({ field }) => (
                    <FormItem className="flex flex-col">
                      ...
                      <Popover open={isCustomerPopoverOpen} onOpenChange={setIsCustomerPopoverOpen}>
                        <PopoverTrigger asChild>
                          <FormControl>
                            <Button variant="outline" className={cn("w-full justify-between", !field.value && "text-muted-foreground")} disabled={isCustomerSelectionDisabled}>
                  ...
                  {displayCustomer && (
                    <div className="mt-2 text-sm text-muted-foreground p-3 bg-muted/50 rounded-md">
                      ...
                      <p className="whitespace-pre-wrap">{displayCustomer.taxAddress}</p>
              ...
              <Card className="bg-muted/10 border-dashed flex flex-col items-center justify-center text-center p-6">
                ...
                <p className="text-sm text-muted-foreground">ระบุเงื่อนไขการแยกจ่ายหรือเครดิต<br/>ในขั้นตอนการส่งตรวจสอบ</p>
              </Card>
            </div>
            ...
            <Card>
                  ...
                  <Popover open={isQtSearchOpen} onOpenChange={setIsQtSearchOpen}>
                    <PopoverTrigger asChild>
                      <Button type="button" variant="outline" size="sm" className="h-8" onClick={() => loadAllDocs('QUOTATION')} disabled={isLocked}>
                        <FileSearch className="mr-2 h-3 w-3" /> ใบเสนอราคา
                      </Button>
                    </PopoverTrigger>
                    <PopoverContent className="w-80 p-0" align="start">
                      <div className="p-2 border-b">
                        <Input placeholder="ค้นหา..." value={qtSearchQuery} onChange={e=>setQtSearchQuery(e.target.value)} />
                      </div>
                      <ScrollArea className="h-60">
                        {isSearchingQt ? <Loader2 className="animate-spin m-4" /> : getFilteredDocs(allQuotations, qtSearchQuery).map(q => (
                  ...
                  <Popover open={isBillSearchOpen} onOpenChange={setIsBillSearchOpen}>
                    <PopoverTrigger asChild>
                      <Button type="button" variant="outline" size="sm" className="h-8" onClick={() => loadAllDocs('BILLS')} disabled={isLocked}>
                        <FileStack className="mr-2 h-3 w-3" /> บิลขาย
                      </Button>
                    </PopoverTrigger>
                    <PopoverContent className="w-80 p-0" align="start">
                      <Tabs value={billSearchType} onValueChange={(v: any) => setBillSearchType(v)}>
                        <TabsList className="w-full rounded-none">
                          <TabsTrigger value="DELIVERY_NOTE" className="flex-1 text-[10px]">ใบส่งของ</TabsTrigger>
                          <TabsTrigger value="TAX_INVOICE" className="flex-1 text-[10px]">ใบกำกับ</TabsTrigger>
                        </TabsList>
                        <div className="p-2 border-b">
                          <Input placeholder="ค้นหา..." value={billSearchQuery} onChange={e => setBillSearchQuery(e.target.value)} />
                        </div>
                        <ScrollArea className="h-60">
                          {isSearchingBills ? <Loader2 className="animate-spin m-4" /> : getFilteredDocs(allBills, billSearchQuery, billSearchType).map(d => (
                            <Button key={d.id} variant="ghost" className="w-full justify-start h-auto py-2 px-3 border-b text-left" onClick={() => handleFetchFromDoc(d)}>
                              <div className="flex flex-col">
                                <span className="font-semibold">{d.docNo}</span>
                                <span className="text-[10px]">{d.customerSnapshot?.name}</span>
                              </div>
                            </Button>
                          ))}
                        </ScrollArea>
              ...
              <CardContent>
                      ...
                      <TableRow key={field.id}>
                        ...
                        <TableCell>
                          <FormField control={form.control} name={`items.${index}.quantity`} render={({ field }) => (
                            <Input type="number" step="any" className="text-right" value={(field.value ?? 0) === 0 ? "" : field.value} onChange={(e) => { const v = e.target.value === '' ? 0 : Number(e.target.value); field.onChange(v); form.setValue(`items.${index}.total`, v * form.getValues(`items.${index}.unitPrice`)); }} disabled={isLocked} />
                          )}/>
                        </TableCell>
                        <TableCell>
                          <FormField control={form.control} name={`items.${index}.unitPrice`} render={({ field }) => (
                            <Input type="number" step="any" className="text-right" value={(field.value ?? 0) === 0 ? "" : field.value} onChange={(e) => { const v = e.target.value === '' ? 0 : Number(e.target.value); field.onChange(v); form.setValue(`items.${index}.total`, v * form.getValues(`items.${index}.quantity`)); }} disabled={isLocked} />
                          )}/>
                        </TableCell>
                        <TableCell className="text-right font-medium">{formatCurrency(form.watch(`items.${index}.total`))}</TableCell>
                        <TableCell>
                          <Button type="button" variant="ghost" size="icon" onClick={() => remove(index)} disabled={isLocked}>
                ...
                <Button type="button" variant="outline" size="sm" className="mt-4" onClick={() => append({description: '', quantity: 1, unitPrice: 0, total: 0})} disabled={isLocked}>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <FormField control={form.control} name="notes" render={({ field }) => (
                    <FormItem><FormControl><Textarea placeholder="รายละเอียด..." rows={4} {...field} disabled={isLocked}/></FormControl></FormItem>
                  )} />
                  <div className="grid grid-cols-1 gap-4">
                    <FormField control={form.control} name="issueDate" render={({ field }) => (
                      <FormItem><FormLabel>วันที่</FormLabel><FormControl><Input type="date" {...field} value={field.value ?? ''} disabled={isLocked} /></FormControl></FormItem>
                    )} />
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <FormField control={form.control} name="senderName" render={({ field }) => (
                      <FormItem><FormLabel>ผู้ส่งของ</FormLabel><FormControl><Input {...field} value={field.value ?? ''} disabled={isLocked} /></FormControl></FormItem>
                    )} />
                    <FormField control={form.control} name="receiverName" render={({ field }) => (
                      <FormItem><FormLabel>ผู้รับของ</FormLabel><FormControl><Input {...field} value={field.value ?? ''} disabled={isLocked} /></FormControl></FormItem>
                    )} />
                  </div>
                </CardContent>
              </Card>
              <div className="space-y-4 p-6 border rounded-lg bg-muted/30">
                ...
                <div className="flex justify-between items-center text-sm">
                  ...
                  <FormField control={form.control} name="discountAmount" render={({ field }) => (
                    <Input type="number" step="any" className="w-32 text-right bg-background h-8" {...field} value={(field.value ?? 0) === 0 ? "" : field.value} onChange={(e) => field.onChange(e.target.value === "" ? 0 : Number(e.target.value))} disabled={isLocked} />
                  )}/>
                </div>
                <Separator/>
        ...
        <DialogContent className="sm:max-w-2xl max-h-[95vh] flex flex-col p-0 overflow-hidden">
          ...
          <DialogFooter className="bg-muted/30 p-6 border-t">
            ...
            <Button 
                ...
                disabled={isProcessing || (remainingAmount > 0.01 && !recordRemainingAsCredit) || (suggestedPayments.some(p => p.amount > 0 && !p.accountId))}
            >
