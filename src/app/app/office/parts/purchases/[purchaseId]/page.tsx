...
import { AlertCircle, ArrowLeft, Printer, ExternalLink, Edit, Loader2, Camera, Trash2 } from "lucide-react";
...
// Component to manage photo uploads and deletions
function BillPhotoManager({ document }: { document: PurchaseDoc }) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const currentPhotoCount = document.billPhotos?.length ?? 0;
    const availableSlots = 4 - currentPhotoCount;

    if (filesToUpload.length > availableSlots) {
        toast({ variant: 'destructive', title: 'Too many files', description: `You can only upload ${availableSlots} more photos.` });
    ...
    const validFiles = filesToUpload.filter(file => {
        if (file.size > 5 * 1024 * 1024) { // 5MB
            toast({ variant: 'destructive', title: `File too large: ${file.name}`, description: 'Max size is 5MB.' });
            return false;
        }
        return true;
    });
    ...
    try {
        const docRef = doc(db, 'purchaseDocs', document.id);
        const uploadPromises = validFiles.map(async (file) => {
            const storageRef = ref(storage, `purchases/${document.id}/bills/${Date.now()}-${file.name}`);
        ...
        await updateDoc(docRef, {
            billPhotos: arrayUnion(...photoURLs),
            updatedAt: serverTimestamp(),
        });

        toast({ title: "อัปโหลดรูปบิลสำเร็จ", description: `${validFiles.length} photo(s) added.` });
    } catch (error: any) {
  ...
  const handleDeletePhoto = async (urlToDelete: string) => {
    ...
    try {
        const docRef = doc(db, 'purchaseDocs', document.id);
        await updateDoc(docRef, {
            billPhotos: arrayRemove(urlToDelete),
            updatedAt: serverTimestamp(),
        });
        toast({ title: 'Photo removed' });
    } catch (error: any) {
        toast({ variant: 'destructive', title: 'Failed to remove photo', description: error.message });
    } finally {
        setIsDeleting(null);
    }
  };
  ...
  return (
    <Card className="print:hidden">
      <CardHeader>
        <CardTitle>แนบรูปบิล ({document.billPhotos?.length ?? 0}/4)</CardTitle>
        <CardDescription>อัปโหลดรูปภาพของบิลหรือใบเสร็จเพื่อใช้อ้างอิง</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="mb-4">
          <label htmlFor="bill-photo-upload" className="block">
            <Button asChild variant="outline" disabled={isUploading || (document.billPhotos?.length ?? 0) >= 4}>
            ...
            <Input 
              ...
              disabled={isUploading || (document.billPhotos?.length ?? 0) >= 4}
            />
          </label>
        </div>
        ...
        {document.billPhotos && document.billPhotos.length > 0 && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {document.billPhotos.map((url) => (
              <div key={url} className="relative group">
                <a href={url} target="_blank" rel="noopener noreferrer">
                  <Image src={url} alt="Bill photo" width={200} height={200} className="rounded-md object-cover w-full aspect-square" />
                </a>
                <Button 
                  variant="destructive" 
                  size="icon" 
...
function PurchaseViewPageContent() {
    ...
    return (
        <div className="space-y-6">
            ...
            <PurchaseDocView document={document} />
            <BillPhotoManager document={document} />
        </div>
    );
}
