...
export default function AccountingInboxPage() {
  ...
  const hasPermission = useMemo(() => profile?.role === 'ADMIN' || profile?.department === 'MANAGEMENT' || profile?.department === 'FINANCE' || profile?.department === 'ACCOUNTING', [profile]);
  ...
  useEffect(() => {
    ...
    const unsubClaims = onSnapshot(claimsQuery, 
      ...
      (error) => {
        ...
        toast({ variant: 'destructive', title: "Could not load inbox", description: error.message });
        setLoading(false);
      }
    );
