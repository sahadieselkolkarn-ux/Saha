  ...
  useEffect(() => {
    const fetchData = async () => {
        ...
        try {
            ...
            // Filter out users who do not require attendance tracking
            const usersToProcess = allUsersData.filter(user => 
                user.hr?.payType !== 'NOPAY' && user.hr?.payType !== 'MONTHLY_NOSCAN'
            );
